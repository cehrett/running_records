...
# parser.add_argument("-b", "--use_bpe", help="Use byte-pair encoding",
...
space = {
    ...
    # hp.lognormal('enc_dropout',-2.5,1),
    'enc_dropout': 0.06174325431277418,
    # hp.lognormal('dec_dropout',-2.5,1),
    'dec_dropout': 0.0059501976431332645,
    'clip': 1,
    # 'bpe_vocab_size': scope.int(hp.qloguniform('bpe_vocab_size',6,8,200)),
    'decode_trg': True,
    'early_stop': 20,
    'overfit': False,  # Used for dev, set to false for real training,
    # Track whether we use a phonetic dataset. This is important due to column selection,
    'phonetics': True
}
...
def hyperopt_train_test(params):
    ...
    # Update params
    params['hid_dim'] = lcm(
        params['enc_heads'], params['dec_heads']) * params['hid_dim_nheads_multiplier']
    ...
    # TODO: Replace with some post-processing for Weights & Biases.
    if params['overfit']:
        loss = train_loss
    else:
        loss = test_loss
...
def main():
    wandb.init()
    hyperopt_train_test(wandb.config)
